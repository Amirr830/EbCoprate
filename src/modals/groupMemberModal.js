...
export default function GroupMembersModal(props) {
    ...
    useEffect(() => {
        if (props.show)
            getGroupMembers(props.groupCode)
    }, [props.show])

    var [members, setMembers] = useState([])
    var [isLoading, setLoading] = useState(true)
    var getGroupMembers = (groupCode) => {
        setLoading(true)
    ...
    var deleteMember = (userCode, groupCode) => {
        answerModal.show('آیا از حذف این عضو اطمینان دارید؟', () => {
                                    ...
                                    return <div key={index} >
                                        <div className="card d-flex py-2 flex-row justify-content-center align-items-center">
                                            ...
                                            <CgClose className="text-danger aPointer mx-2" size={25} onClick={() => {
                                                deleteMember(item.userCode, item.groupCode)
                                            }} />
                                        </div>
                                    </div>
                                })

...
import StationMultiSelector from "./stationMultiSelector";

export default function AddFrequentAddress(props) {
...
function MapController(props) {
    ...
    var AddFrequentAddress = () => {
        ...
        if (selectStList?.length == 0) {
            toast.Error('کد ایستگ1اه را وارد نمایید')
            return
        }

        var params = {
            stCode: selectStList.join(','),
            title,
            lat: center?.lat,
            lng: center?.lng,
            fullAddress: currentAddress?.address,
            frequentCode: props?.params?.frequentCode
        }

        AxiosPrivate.post(endpoints.pointIOFrequentAddress, params)
            .then((res) => {
                ...
                <div className="row g-2">
                    <div className="col-4 pt-2">
                        <input className="form-control shadow mb-2 border-secondary "
                            placeholder="عنوان مقصد"
                            value={title}
                            onChange={(e) => { setTitle(e.target.value) }}
                        />
                    </div>
                    <StationMultiSelector
                        selectedSt={selectStList}
                        onStList={(stList) => {
                            setSelectedStList(stList)
                        }}>
                        <div className="col-8 overflow-x-auto d-flex py-2">

                            <button className="btn btn-success" >
                                افزودن ایستگاه
                            </button>
                            {
                                selectStList?.map((item, index) => {
                                    return <button key={index}
                                        className="btn btn-dark  text-center mx-1 p-1 " style={{ width: '3rem' }} disabled >
                                        {item}
                                    </button>
                                })
                            }

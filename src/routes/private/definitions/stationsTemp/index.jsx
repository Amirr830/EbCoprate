...
export default function Stations() {
  ...
  var getStations = () => {
    setLoading(true)
    AxiosPrivate.get(endpoints.stationsV2, { params: params }).then(res => {
      setStations(res.data)
      setLoading(false)
    })
  }

  var deleteStation = (stCode) => {
    answerModal.show(`آیا از حذف ایستگاه ${stCode} مطمئن هستید؟`, () => {
      setLoading(true)
      AxiosPrivate.delete(endpoints.stationsV2, { data: { stCode } }).then(res => {
        getStations()
      })
    }, () => {
                    ...
                    {stations.map((item, index) => {
                      return <AddStation
                        ...
                        <tr className="aPointer">
                          ...
                          <td>
                            <CgClose className="aPointer text-hover " size={30} onClick={(e) => {
                              e.stopPropagation();
                              deleteStation(item?.stCode)
                            }} />
                          </td>
                        </tr>
                      </AddStation>

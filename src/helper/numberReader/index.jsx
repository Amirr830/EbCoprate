...
import _1000 from './1000.wav'
import _1000o from './1000_.wav'
...
export default function NumberReader(vTemp) {
    var playList = generatePlayList(vTemp);
    playList.unshift('shomare')
    // playList = ['shomare',...playList]
    preloadSounds(playList);
    play(playList);
};

const generatePlayList = (vTemp) => {
    ...
    if (h > 0) {
        playList.push(...generatePlayList(h))
    }
    if (h > 0 && (s > 0 || d > 0 || y > 0)) playList.push('_1000o');
    else if (h > 0) playList.push('_1000');

    if (s > 0 && (d > 0 || y > 0)) playList.push('_' + s * 100 + 'o');
    else if (s > 0) playList.push('_' + s * 100);

    if (d > 0 && y > 0) playList.push('_' + d * 10 + 'o');
    else if (d < 20 && d > 10 && y == 0) playList.push('_' + d);
    else if (d > 0) playList.push('_' + d * 10);

    if (y > 0) playList.push('_' + y);
    console.log(playList)
    return playList;
};

const preloadSounds = (playList) => {
    playList.forEach((sound) => {
        const audio = new Audio(sounds[sound]);
        audio.load(); // پیش‌لود فایل‌های صوتی
    });
};
const play = (playList) => {
    if (playList.length === 0) return;

    const audio = new Audio(sounds[playList[0]]);
    const overlapTime = 200; // زمان شروع فایل بعدی قبل از پایان (بر حسب میلی‌ثانیه)

    const handleTimeUpdate = () => {
        if (audio.duration - audio.currentTime <= overlapTime / 1000) {
            // حذف لیسنر برای جلوگیری از تکرار
